...
export default function HeroSection() {
  ...
  return (
    <section className="hero-viewport relative flex flex-col overflow-hidden max-lg:min-h-0 md:min-h-[80vh] lg:min-h-screen lg:justify-center">
      <div
        className="hero-home-bg absolute inset-0 bg-no-repeat"
        style={{
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col justify-start px-4 pb-5 pt-20 sm:px-6 sm:pb-6 sm:pt-24 lg:justify-center lg:px-8 lg:py-32">
        <div className="w-full max-w-xl text-left sm:max-w-2xl">
          ...
          <div className="animate-fade-up mt-7 flex w-full max-w-md flex-row gap-2 delay-300 sm:mt-8 sm:max-w-none sm:flex-wrap sm:gap-3">
            <a
              href={youtubeUrl}
              target="_blank"
              rel="noreferrer"
              className="touch-target inline-flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-2xl bg-youtube-red px-3 py-3.5 text-base font-semibold text-white shadow-xl shadow-youtube-red/30 transition-all hover:-translate-y-0.5 hover:bg-red-600 sm:w-auto sm:flex-none sm:gap-2 sm:px-6"
            >
              <Youtube size={20} className="shrink-0" aria-hidden />
              <span className="truncate">{t('home.ctaWatch')}</span>
            </a>
            <a
              href={subscribeUrl || YOUTUBE_SUBSCRIBE_URL}
              target="_blank"
              rel="noreferrer"
              className="touch-target inline-flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-2xl border border-gold/40 bg-gold/10 px-3 py-3.5 text-base font-semibold text-gold backdrop-blur transition-all hover:-translate-y-0.5 hover:border-gold hover:bg-gold/20 sm:w-auto sm:flex-none sm:gap-2 sm:px-6"
            >
